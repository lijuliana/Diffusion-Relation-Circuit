...
The submission PDF is in [`paper/`](paper/).
...
├── checkpoints/                  # trained PixArt-mini weights at 11 training epochs (symlinks)
├── t5_embedding_cache.pt         # cached T5-XXL embeddings for all training prompts (symlink)
├── data/
│   ├── shapes_dataset_pilot.pth  # synthetic 2-color × 3-shape × 8-relation training data (symlink)
│   └── results_csv/              # CSVs of per-experiment results (consumed by figure scripts)
├── figures/                      # final paper figures (PDF + PNG)
└── paper/                        # paper PDF, .tex, .bib (ICML and NeurIPS variants)
```
...
Every figure in the paper is generated by a script in [`experiments/`](experiments/). Each script reads a CSV from [`data/results_csv/`](data/results_csv/) and writes PNG + PDF into [`figures/`](figures/).
