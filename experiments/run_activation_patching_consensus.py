...
def main():
    ...
    for layer_idx, head_idx in test_heads:
        ...
        for pi, (clean_prompt, corrupt_prompt, clean_scene, corrupt_scene) in enumerate(pairs):
            ...
            clean_acc   = float(eval_clean["spatial_relationship"].mean())
            corrupt_acc = float(eval_corrupt["spatial_relationship"].mean())
            patched_acc = float(eval_patched["spatial_relationship"].mean())
