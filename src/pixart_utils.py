...
class PixArtAlphaPipeline_custom(PixArtAlphaPipeline):
    """
    Notebook / local-checkpoint compatibility.

    HuggingFace ``PixArtAlphaPipeline`` resolution binning only allows
    ``transformer.config.sample_size`` in ``{32, 64, 128}`` (256 / 512 / 1024 px
    hub models).  DiT-mini training uses smaller ``image_size`` (e.g. 128 px →
    sample_size 16), which otherwise raises ``ValueError: Invalid sample size``.
    """

    def __call__(self, *args: Any, **kwargs: Any):
        ss = getattr(self.transformer.config, "sample_size", None)
        if ss is not None and ss not in (32, 64, 128):
            kwargs["use_resolution_binning"] = False
        # Local workflows often run with text_encoder=None and cached prompt_embeds.
        # Diffusers CFG path may still expect negative embeds; synthesize them here.
        prompt_embeds = kwargs.get("prompt_embeds", None)
        guidance_scale = float(kwargs.get("guidance_scale", 1.0))
        if (
            prompt_embeds is not None
            and guidance_scale > 1.0
            and kwargs.get("negative_prompt_embeds", None) is None
        ):
            kwargs["negative_prompt_embeds"] = torch.zeros_like(prompt_embeds)
            # Diffusers forbids passing both negative_prompt and negative_prompt_embeds.
            kwargs["negative_prompt"] = None
            prompt_mask = kwargs.get("prompt_attention_mask", None)
            if prompt_mask is not None and kwargs.get("negative_prompt_attention_mask", None) is None:
                kwargs["negative_prompt_attention_mask"] = torch.ones_like(prompt_mask)
        kwargs.setdefault("clean_caption", False)
