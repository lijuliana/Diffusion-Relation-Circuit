...
def build_heldout_generalization_prompts(
    prompt_scene_info_all_df: pd.DataFrame,
    embedding_cache: dict,
    *,
    exclude_pairs: List[Tuple[str, str, str, str]] | None = None,
    n_per_relation: int = 3,
    max_prompts: int | None = 24,
) -> Tuple[List[str], List[Dict[str, Any]], pd.DataFrame]:
    """
    Build held-out validation prompts by excluding designated object-pair templates.

    This is intended for "validate-only" circuit checks where prompt templates used
    during training-style evaluations should not appear in the validation set.
    """
    required_cols = {"prompt", "color1", "shape1", "color2", "shape2", "spatial_relationship"}
    missing = required_cols - set(prompt_scene_info_all_df.columns)
    if missing:
        raise KeyError(f"prompt_scene_info_all_df missing required columns: {sorted(missing)}")

    exclude_pairs = exclude_pairs or [("blue", "circle", "red", "square")]
    exclude_pairs_norm = {
        (str(c1), str(s1), str(c2), str(s2))
        for c1, s1, c2, s2 in exclude_pairs
    }

    work_df = prompt_scene_info_all_df.copy()
    keep_mask = ~work_df.apply(
        lambda r: (str(r["color1"]), str(r["shape1"]), str(r["color2"]), str(r["shape2"])) in exclude_pairs_norm,
        axis=1,
    )
    heldout_df = work_df[keep_mask].copy()

    # Keep only prompts with precomputed embeddings.
    cache_prompt_set = {
        key.split("::", 1)[1]
        for key in embedding_cache.keys()
        if key and "::" in key and key != ""
    }
    heldout_df = heldout_df[heldout_df["prompt"].isin(cache_prompt_set)].copy()
    if heldout_df.empty:
        return [], [], heldout_df

    heldout_df = (
        heldout_df.groupby("spatial_relationship", group_keys=False)
        .head(int(max(1, n_per_relation)))
        .reset_index(drop=True)
    )
    if max_prompts is not None:
        heldout_df = heldout_df.head(int(max_prompts)).copy()

    prompts = heldout_df["prompt"].tolist()
    scene_infos = [
        {
            "color1": str(r.color1),
            "shape1": str(r.shape1),
            "color2": str(r.color2),
            "shape2": str(r.shape2),
            "spatial_relationship": str(r.spatial_relationship),
        }
        for r in heldout_df.itertuples()
    ]
    return prompts, scene_infos, heldout_df
